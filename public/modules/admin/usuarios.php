...
<table class="table"><tr><th>ID</th><th>Nombre</th><th>Email</th><th>Rol</th><th>Estado</th><th>Creado</th><th>Acciones</th></tr><?php foreach($users as $u): ?><tr><td><?= (int)$u['id'] ?></td><td><?= htmlspecialchars($u['nombre']) ?></td><td><?= htmlspecialchars($u['email']) ?></td><td><?php if ($u['rol']==='admin') { echo ui_badge('Administrador', 'info'); } elseif ($u['rol']==='analista') { echo ui_badge('Analista', 'default'); } else { echo ui_badge('Visualizador', 'warning'); } ?></td><td><?= $u['estado']==='activo' ? ui_badge('Activo', 'success') : ui_badge('Inactivo', 'danger') ?></td><td><?= htmlspecialchars($u['created_at']) ?></td><td><form style="display:inline" method="post"><input type="hidden" name="action" value="toggle"><input type="hidden" name="id" value="<?= (int)$u['id'] ?>"><input type="hidden" name="estado" value="<?= $u['estado']==='activo'?'inactivo':'activo' ?>"><button class="btn btn-muted">Activar/Inactivar</button></form> <form style="display:inline" method="post"><input type="hidden" name="action" value="reset"><input type="hidden" name="id" value="<?= (int)$u['id'] ?>"><button class="btn">Reset pass</button></form></td></tr><?php endforeach; ?></table>
<?php
